...
SwitchReport *N64Controller::getSwitchReport()
{
    ...
    uint8_t scaledX = xPos > 0 ? xPos * (SWITCH_JOYSTICK_MID - 1) / _maxX :
                                 xPos * -SWITCH_JOYSTICK_MID / _minX;
    ...
    uint8_t scaledY = yPos > 0 ? yPos * (SWITCH_JOYSTICK_MID - 1) / _maxY :
                                 yPos * -SWITCH_JOYSTICK_MID / _minY;
