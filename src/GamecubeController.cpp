...
uint8_t GamecubeController::getScaledAnalogAxis(uint8_t axisPos,
                                                uint8_t *minAxis,
                                                uint8_t *maxAxis)
{
    *maxAxis = axisPos > *maxAxis ? axisPos : *maxAxis;
    *minAxis = axisPos < *minAxis ? axisPos : *minAxis;
    return axisPos > 0x80 ? SWITCH_JOYSTICK_MID - 1 +
                                (axisPos - SWITCH_JOYSTICK_MID) /
                                    (float)(*maxAxis - SWITCH_JOYSTICK_MID) *
                                    (float)SWITCH_JOYSTICK_MID :
                            SWITCH_JOYSTICK_MID -
                                (SWITCH_JOYSTICK_MID - axisPos) /
                                    (float)(SWITCH_JOYSTICK_MID - *minAxis) *
                                    (float)SWITCH_JOYSTICK_MID;
}
...
SwitchReport *GamecubeController::getSwitchReport()
{
    ...
    _switchReport.buttons =
        ...
        (GC_MASK_Y & _controllerState[0] ?
             (_isOneToOne ? SWITCH_MASK_Y : SWITCH_MASK_R3) :
             0) |
        (GC_MASK_X & _controllerState[0] ? SWITCH_MASK_X : 0) |
        (GC_MASK_B & _controllerState[0] ?
             (_isOneToOne ? SWITCH_MASK_B : SWITCH_MASK_Y) :
             0) |
    ...
    _switchReport.lx =
        getScaledAnalogAxis(_controllerState[2], &_minAnalogX, &_maxAnalogX);
    _switchReport.ly = getScaledAnalogAxis(
        (1 - _controllerState[3] / 255.) * 255., &_minAnalogY, &_maxAnalogY);
    _switchReport.rx =
        getScaledAnalogAxis(_controllerState[4], &_minCX, &_maxCX);
    _switchReport.ry = getScaledAnalogAxis(
        (1 - _controllerState[5] / 255.) * 255., &_minCY, &_maxCY);
